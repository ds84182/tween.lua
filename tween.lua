...
local function newCallingTween(time, func, easing, callback, args)
	local self = {
	time = time,
	func = func,
	easing = easing,
...
local function easeWithCallingTween(self, func)
	local t,b,c,d
	t,b,c,d = self.running, 0, 1, self.time
	func(self.easing(t,b,c,d))
end

local function updateTween(self, dt)
	self.running = self.running + dt
	if self.subject then easeWithTween(self, self.subject, self.target, self.initial) else easeWithCallingTween(self,self.func) end
end
...
local function finishTween(self)
	if self.subject then copyTables(self.subject, self.target) else self.func(1,true) end
...
-- creates a tween where it calls the method func
function tween.startCallingTween(time, func, easing, callback, ...)
	easing = getEasingFunction(easing)
	return newCallingTween(time, func, easing, callback, {...})
end
...
if game and game.Workspace then Spawn(function() while true do tween.update(wait()) end end) end -- for support inside roblox modulescripts
